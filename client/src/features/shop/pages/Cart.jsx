...
const Cart = () => {
  ...
  const getCart = () => {
    ...
    for (const key in cartItems) {
      ...
      const [productId, weight] = key.includes("-")
        ? key.split("-")
        : [key, ""];
  ...
  const placeOrder = async () => {
    try {
      ...
      if (paymentOption === "COD") {
        ...
        if (!data.success)
          return toast.error(data.message || "Payment init failed");
        ...
        if (!res) return toast.error("Failed to load payment SDK");
        ...
        <div className="flex-1 w-full lg:w-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {cartArray.map((product, index) => (
              <div
                key={index}
                className="p-5 flex flex-col md:grid md:grid-cols-[3fr_1fr_1fr_auto] gap-6 items-center hover:bg-gray-50/50 transition-colors"
              >
                {/* Product Info */}
            ...
            <div className="space-y-6">
                ...
                <div className="text-gray-600 text-sm leading-relaxed pr-2">
                  {selectedAddress ? (
                    ...
                    <p className="text-orange-500 flex items-center gap-2">
                      <span className="text-lg">⚠️</span> Please select an
                      address
                    </p>
                  )}
                </div>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden text-sm animate-in fade-in slide-in-from-top-2">
                    <div className="max-h-64 overflow-y-auto no-scrollbar">
                      {addresses.map((addr) => (
                        <div
                          key={addr._id}
                          className="p-3 hover:bg-gray-50 border-b border-gray-50 transition-colors cursor-pointer relative group/item"
                        >
                          <div
                            onClick={() => {
                    ...
                    <button
                      ...
                      className="w-full p-3 text-center text-primary font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                    >
              ...
              <div className="border-t border-gray-100 pt-4 space-y-3">
                ...
                <div className="flex justify-between text-gray-600">
                  ...
                  <span>
                    {currency}
                    {((getCartAmount() * 2) / 100).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between text-xl font-bold text-gray-900 pt-2 border-t border-gray-100 mt-2">
                  ...
                  <span>
                    {currency}
                    {(getCartAmount() * 1.02).toLocaleString()}
                  </span>
                </div>
              </div>
